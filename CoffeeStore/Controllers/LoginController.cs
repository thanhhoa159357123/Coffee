    ...
    public class LoginController : Controller
    {
        ...
        public IActionResult Login(string phoneNumber, string? returnUrl)
        {
            if (_repository.CheckPhoneNumber(phoneNumber))
            {
                ...
                var user = _repository.GetCustomerByPhoneNumber(phoneNumber);
                if (user != null)
                {
        ...
        public IActionResult SignUp(Customer newCustomer)
        {
            ...
            {
                // Kiểm tra giá trị của GioiTinh để đặt giá trị NULL nếu người dùng chọn "Không muốn trả lời"
                if (newCustomer.GioiTinh == "")
                {
                    newCustomer.GioiTinh = null;
                }
                // Kiểm tra xem số điện thoại đã tồn tại trong hệ thống hay chưa
                if (!_repository.CheckPhoneNumber(newCustomer.Phone))
                {
            ...
            // Trả về View với thông tin khách hàng đã nhập để người dùng sửa lại
